...
;;; ------------------- Discovery Schema (DS) and message Schema-conforming Response (SCR) -----------------------------------------
...
(defn get-questioning-budget-left!
  ...
  (if-let [eid (ASCR-exists? pid ds-id)]
    ...
    (do (when ds-id (put-new-ASCR! pid ds-id))
        1.0)))
...
(defn get-ASCR
  "Return the summary DS data structure give the IDs for the project and DS-id.
