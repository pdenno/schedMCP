...
;;; ToDo. We need a separate spec repository for each DS, or better naming in order to avoid name collisions.
...
(s/def ::event-type (s/keys :req-un [::occurrence-assignment ::event-type-id] :opt-un [::periodicity ::event-constituents ::opportunistic?]))
...
(s/def ::timeslot (s/keys :req-un [::availability ::periods ::ts-type-id]))
(s/def ::annotated-timeslots (s/keys :req-un [::comment :timeslots/val]))
(s/def ::availability (s/or :normal :availability/val :annotated ::annotated-availability))
(s/def :availability/val (s/coll-of string? :kind vector?))
(s/def ::annotated-availability (s/keys :req-un [::comment :availability/val]))
...
(s/def :occurrence-assignment/val (s/coll-of string? :type vector?))
...
;;; ToDo: add a timetabled? property to flow and job shop DS processes ????
...
(def timetabling
  {:message-type :DS-INSTRUCTIONS
   ...
   :interview-objective
   (str "Whereas most other DS-INSTRUCTIONS in the process interview focus on characterizing the processes by which the interviewees' enterprise produces product or delivers a service,\n"
...
(defn init-timetabling
  []
  (if (s/valid? :timetabling/DS-message timetabling)
    (when-not (sdb/same-DS-instructions? timetabling)
      (log! :info "Updating timetabling in resources and system DB.")
      ;(sutil/update-resources-DS-json! timetabling)
      (sdb/put-DS-instructions! timetabling))
    (throw (ex-info "Invalid DS message (timetabling)." {}))))

(defstate timetabling-ds
