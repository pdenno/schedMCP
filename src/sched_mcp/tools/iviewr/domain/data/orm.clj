...
(s/def :orm/DS-message (s/keys :req-un [::message-type ::interview-objective ::interviewer-agent ::DS]))
(s/def ::DS (s/keys :req-un [::exhausted? ::inquiry-areas ::areas-we-intend-to-discuss]
                      :opt-un [::duration ::msg-id ::DS-ref ::DS-id]))
(s/def ::DS-id #(= % :data/orm))
...
;;; (s/explain :orm/DS-message orm)
(when-not (s/valid? :orm/DS-message orm)
  (throw (ex-info "Invalid DS (ORM)" {})))
...
(defn combine
  ...
  (reduce (fn [best area-id]
            (let [best-area (dsu/get-object best :inquiry-area-id area-id)
                  more-area (dsu/get-object more :inquiry-area-id area-id)
                  best-fact-ids (dsu/collect-keys-vals best-area :fact-type-id)
                  more-fact-ids (dsu/collect-keys-vals more-area :fact-type-id)
                  fact-inserts   (set/difference more-fact-ids best-fact-ids)
                  fact-replaces  (set/intersection best-fact-ids more-fact-ids)]
              (as-> best ?b
                (reduce (fn [r new-id] (dsu/insert-by-id  r :fact-types (dsu/get-object more :fact-type-id new-id))) ?b fact-inserts)
                (reduce (fn [r new-id] (dsu/replace-by-id r :fact-types :fact-type-id (dsu/get-object more :fact-type-id new-id))) ?b fact-replaces))))
          best
...
(defmethod combine-ds! :data/orm
  ...
  (let [msg-scr (->> (pdb/get-msg-SCR pid tag)
        ...
        start-arg (reduce (fn [r m] (merge r m)) {} msg-scr)
        merged (reduce (fn [r ds] (combine r ds)) start-arg msg-scr)]
    (pdb/put-ASCR! pid
                  :data/orm
