...
(s/def :job-shop-c/DS-message (s/keys :req-un [::message-type ::interview-objective ::interviewer-agent ::DS]))
...
(s/def ::DS (s/keys :req-un [::multiple-production-lines? ::job-level-processes]
                      :opt-un [::msg-id ::DS-ref ::DS-id]))
(s/def ::DS-id #(= % :process/job-shop--classifiable))
...
;;; (s/explain :job-shop-c/DS-message jshopc/job-shop-c)
(def job-shop-c
  ...
  {:message-type :DS-INSTRUCTIONS
   ...
   :DS
   {:DS-id :process/job-shop--classifiable
...
(defn init-job-shop-c
  []
  (if (s/valid? :job-shop-c/DS-message job-shop-c)
    (when-not (sdb/same-DS-instructions? job-shop-c)
      ;(sutil/update-resources-DS-json! job-shop-c)
      (sdb/put-DS-instructions! job-shop-c))
    (throw (ex-info "Invalid DS message (job-shop-c)." {}))))

(defstate job-shop-c-ds
